    ...
    async listResourcesPaged(filters, pagination) {
        ...
        return runRead(async (session) => {
            ...
            const [countResult, dataResult] = await Promise.all([
                session.run(`MATCH (r:Resource) ${where} RETURN count(r) AS total`, params, { timeout: QUERY_TIMEOUT_MS }),
                session.run(`MATCH (r:Resource) ${where} RETURN properties(r) AS r ORDER BY r.name SKIP $offset LIMIT $limit`, params, { timeout: QUERY_TIMEOUT_MS }),
            ]);
