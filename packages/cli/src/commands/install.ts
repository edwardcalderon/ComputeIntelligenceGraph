...
export function generateComposeFile(
  ...
  api:
    ...
    environment:
      ...
      - CIG_BOOTSTRAP_TOKEN=\${CIG_BOOTSTRAP_TOKEN:-}
...
export function generateEnvFile(
  ...
  if (manifest.targetMode === 'host') {
    ...
    if (bootstrapToken) {
      lines.push(`CIG_BOOTSTRAP_TOKEN=${bootstrapToken}`);
    }
    lines.push(`JWT_SECRET=${crypto.randomBytes(64).toString('hex')}`);
    lines.push('');
  }
...
async function runInstall(opts: InstallOptions): Promise<void> {
  ...
  const bootstrapToken = mode === 'self-hosted' ? generateBootstrapToken() : undefined;
