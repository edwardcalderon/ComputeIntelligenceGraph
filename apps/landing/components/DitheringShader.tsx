...
export function DitheringShader({
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (u.u_resolution) g.uniform2f(u.u_resolution, width, height);
      if (u.u_colorBack) g.uniform4fv(u.u_colorBack, hexToRgba(colorBack));
      if (u.u_colorFront) g.uniform4fv(u.u_colorFront, hexToRgba(colorFront));
      if (u.u_shape) g.uniform1f(u.u_shape, DitheringShapes[shape]);
      if (u.u_type) g.uniform1f(u.u_type, DitheringTypes[type]);
      if (u.u_pxSize) g.uniform1f(u.u_pxSize, pxSize);
